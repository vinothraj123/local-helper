...
// ============================================================
// ✅ FIXED WebTimePicker — No hand jump when switching modes
// ============================================================
function WebTimePicker({ label, color, time, onChangeTime }) {
  const parts = (time || "06:00").split(":");
  const h = parts[0] || "06";
  ...
  // ✅ FIX: Compute hand angle based on mode ONLY — no cross-contamination
  const hourAngleDeg = (hNum % 12) * 30 - 90;
  const minuteAngleDeg = mNum * 6 - 90;

  const handAngleDeg = mode === "hour" ? hourAngleDeg : minuteAngleDeg;
  const handRadius =
    mode === "hour" ? (hNum >= 12 ? R_INNER : R_OUTER) : R_OUTER;

  const handX = CX + handRadius * Math.cos(toRad(handAngleDeg));
  const handY = CY + handRadius * Math.sin(toRad(handAngleDeg));
  ...
  // ✅ FIX: Scale click coords to SVG viewBox space — prevents wrong angle on scaled SVG
  const handleClockClick = (e) => {
    e.stopPropagation();
    const svg = e.currentTarget;
    const rect = svg.getBoundingClientRect();
    ...
    if (mode === "hour") {
      ...
      const finalH = isInner ? rawH + 12 : rawH;
      onChangeTime(`${String(finalH).padStart(2, "0")}:${m}`);
      // ✅ Switch to minute mode AFTER updating time — no jump
      setMode("minute");
    } else {
      // ✅ FIX: round to nearest single minute accurately
      const rawM = Math.round(norm / 6) % 60;
      onChangeTime(`${h}:${String(rawM).padStart(2, "0")}`);
      // Stay in minute mode — user can click again for fine tuning
    }
  };
  ...
  // All 60 tick marks around edge
  const ticks = Array.from({ length: 60 }, (_, i) => {
    const a = toRad(i * 6 - 90);
    const isMajor = i % 5 === 0;
    const r1 = isMajor ? R_OUTER + 10 : R_OUTER + 13;
    const r2 = R_OUTER + 16;
    return { i, a, isMajor, r1, r2 };
  });

  // ✅ All 60 minute positions as small dots (non-labeled ones)
  const minuteDots = Array.from({ length: 60 }, (_, i) => {
    if (i % 5 === 0) return null;
    const pos = getPos(i * 6, R_OUTER);
    return { i, pos, isActive: mNum === i };
  }).filter(Boolean);
  ...
  return (
    ...
    >
      ...
      {/* ✅ Hour / Minute toggle — clicking these switches mode WITHOUT changing time */}
      <div
        style={{
          display: "flex",
          alignItems: "center",
      ...
      >
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            setMode("hour"); // ✅ Only mode changes, time stays same → no hand jump
          }}
          style={{
            fontSize: 22,
            fontWeight: 900,
        ...
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            setMode("minute"); // ✅ Only mode changes, time stays same → no hand jump
          }}
          style={{
            fontSize: 22,
            fontWeight: 900,
      ...
      <svg
        width="100%"
        viewBox={`0 0 ${SIZE} ${SIZE}`}
        style={{ cursor: "crosshair", flexShrink: 0, maxWidth: SIZE }}
        onClick={handleClockClick}
        onMouseDown={(e) => e.stopPropagation()}
      >
        {/* Face */}
        ...
        {mode === "hour" &&
          outerHours.map((num, i) => {
            ...
            return (
              <g key={"o" + num}>
                {isActive && (
                  <circle cx={pos.x} cy={pos.y} r={15} fill={color} />
                )}
                <text
                  x={pos.x}
                  y={pos.y}
        ...
        {/* MINUTE: small dots for every minute not on a 5-boundary */}
        {mode === "minute" &&
          minuteDots.map(({ i, pos, isActive }) => (
            <circle
              key={"md" + i}
              cx={pos.x}
              cy={pos.y}
        ...
        {/* MINUTE: labeled (0, 5, 10 ... 55) */}
        {mode === "minute" &&
          minuteLabels.map((num, i) => {
            const pos = getPos(i * 30, R_OUTER);
            const isActive = mNum === num;
            return (
              <g key={"mn" + num}>
        ...
        />
        <circle cx={CX} cy={CY} r={4} fill={color} />
        <circle cx={handX} cy={handY} r={7} fill={color} opacity={0.2} />
        <circle cx={handX} cy={handY} r={4} fill={color} />
      </svg>
