...
// ============================================================
// ✅ FIXED WebTimePicker — Minute clock selection works 100%
// Replace the entire WebTimePicker function with this
// ============================================================
...
function WebTimePicker({ label, color, time, onChangeTime }) {
  ...
  // Increased SVG size for better tap accuracy
  const SIZE = 240;
  const CX = SIZE / 2;
  const CY = SIZE / 2;
  const R_OUTER = 92;
  ...
  // ✅ KEY FIX: Use a larger hit radius (110) for minute clicks
  const handleClockClick = (e) => {
    e.stopPropagation();
    const svg = e.currentTarget;
    const rect = svg.getBoundingClientRect();
    // Scale click coords to SVG coordinate space
    ...
    // ✅ Accept clicks within 110px of center (covers full clock face)
    if (dist > 110) return;
    ...
    if (mode === "hour") {
      ...
      // ✅ FIXED: round to nearest single minute (not nearest 5)
      const rawM = Math.round(norm / 6) % 60;
      onChangeTime(`${h}:${String(rawM).padStart(2, "0")}`);
    }
  };
  ...
  // ✅ All 60 minute dot positions (small dots for every minute)
  const minuteDots = Array.from({ length: 60 }, (_, i) => {
    if (i % 5 === 0) return null; // labeled ones handled separately
    const pos = getPos(i * 6, R_OUTER);
    const isActive = mNum === i;
    return { i, pos, isActive };
  }).filter(Boolean);
  ...
  return (
    ...
    >
      ...
      {/* ✅ Clock SVG — width:100% so it scales with container */}
      <svg
        width="100%"
        viewBox={`0 0 ${SIZE} ${SIZE}`}
      ...
      >
        ...
        {/* Hour mode: inner ring separator */}
        {mode === "hour" && (
          <circle
            cx={CX}
            cy={CY}
        ...
        {/* HOUR MODE: outer hours (0/12, 1–11) */}
        {mode === "hour" &&
          outerHours.map((num, i) => {
            const pos = getPos(i * 30, R_OUTER);
            const hVal = num === 12 ? 0 : num;
        ...
        {/* ✅ MINUTE MODE: small dots for every non-labeled minute */}
        {mode === "minute" &&
        ...
        {/* ✅ MINUTE MODE: labeled positions (0, 5, 10, ... 55) */}
        {mode === "minute" &&
          minuteNumbers.map((num, i) => {
            const pos = getPos(i * 30, R_OUTER);
            const isActive = mNum === num;
        ...
        />
        <circle cx={CX} cy={CY} r={4} fill={color} />
        <circle cx={handX} cy={handY} r={7} fill={color} opacity={0.25} />
